...
def get_serial_from_license_str(data: IO[str]) -> str:
	for license in [l for l in json.load(data) if 'Headless' in l['product']]:
		return license['serial']
	raise ValueError(f'No "Headless" license found')

def get_serial_from_license(path: Union[Path, str]) -> str:
	...
	"""
	with open(path) as f:
		return get_serial_from_license_str(f)

def download_headless(serial: str, output_path: Optional[str], dev: bool = False) -> str:
	...
	if output_path is None:
		output_path = os.path.basename(urlparse(req_url).path)
...
def download_and_install(
  serial: Optional[str] = None, downloaddir: Optional[str] = None, dev: bool = False, clean: bool = False, install: bool = False,
  installdir: str = '/usr/local/bin'
):
	env_headless_lic = 'BN_HEADLESS_LICENSE'
	env_lic = 'BN_LICENSE'
	if serial is None:
		if env_lic in os.environ:
			license_str = os.environ[env_lic]
			serial = get_serial_from_license_str(StringIO(license_str))
		else:
			if env_headless_lic in os.environ:
				license_path = os.environ[env_headless_lic]
			else:
