...
public class DecompilerNestedLayout extends AbstractBBGraphLayout {
  ...
  public DecompilerNestedLayout(BasicBlockGraph graph, String name) {
    this(graph, name, true);
  }

  private DecompilerNestedLayout(BasicBlockGraph graph, String name, boolean initialize) {
    super(graph, name);
    if (initialize) {
      initialize();
    }
  ...
  @Override
  public EdgeLabel<BasicBlockVertex, BasicBlockEdge> getEdgeLabelRenderer() {
  ...
  private anvill.plugin.anvillpatchgraph.layout.DNLayoutOptions getLayoutOptions() {
  ...
  protected GridLocationMap<BasicBlockVertex, BasicBlockEdge> performInitialGridLayout(
      VisualGraph<BasicBlockVertex, BasicBlockEdge> jungGraph) throws CancelledException {
    ...
    try {
      ...
      BasicBlockVertex aVertex = jungGraph.getVertices().iterator().next();
      ...
      BlockGraph ingraph = buildCurrentBBGraph(program, jungGraph, monitor);
      if (ingraph == null) {
        throw new RuntimeException("Unable to initialize: " + ifc.getLastMessage());
      }
    ...
    BasicBlockVertex vertex = getVertex(jungGraph, entryPoint);
  ...
  private void labelEdges(
    ...
    for (BasicBlockEdge e : edges) {
      ...
      BasicBlockVertex start = e.getStart();
      BasicBlockVertex end = e.getEnd();
      ...
      Collection<BasicBlockEdge> outEdges = jungGraph.getOutEdges(start);
  ...
  protected Map<BasicBlockEdge, List<Point2D>> positionEdgeArticulationsInLayoutSpace(
    ...
    Map<BasicBlockEdge, List<Point2D>> newEdgeArticulations = new HashMap<>();
    ...
    for (BasicBlockEdge e : edges) {
      ...
      BasicBlockVertex startVertex = e.getStart();
      BasicBlockVertex endVertex = e.getEnd();
  ...
  private List<Point2D> routeUpwardLoop(
    ...
    BasicBlockVertex rightmostVertex = getRightmostVertex(interlopers);
  ...
  private void lighten(BasicBlockEdge e) {
  ...
  private BasicBlockVertex getRightmostVertex(
    ...
    BasicBlockVertex v = getRightmostVertex(points);
    return v;
  }

  private BasicBlockVertex getRightmostVertex(Collection<Vertex2d> points) {
  ...
  private GridLocationMap<BasicBlockVertex, BasicBlockEdge> assignCoordinates(
    ...
    for (BasicBlockVertex vertex : vertices) {
   ...
   * report vertex offsets from the appropriate side and top/bottom
   */
  ...
  private class Vertex2dFactory {
    ...
    private Map<BasicBlockVertex, Point2D> vertexLayoutLocations;
    ...
    Vertex2d get(BasicBlockVertex v) {
      return cache.get(v);
    }

    Vertex2d get(int rowIndex, int columnIndex) {

      Row<BasicBlockVertex> row = layoutToGridMap.row(rowIndex);
      BasicBlockVertex v = row.getVertex(columnIndex);
      if (v == null) {
        return null;
      }
      return get(v);
  ...
  /**
   * A class that represents 2D information about the contained vertex, such as location, bounds,
   * row and column of the layout grid.
   */
  private class Vertex2d {

    private BasicBlockVertex v;
    private Row<BasicBlockVertex> row;
  ...
  private class DecompilerBlockGraph extends DecompilerBlock {
    ...
    @Override
    DecompilerBlock getBlock(BasicBlockVertex vertex) {
    ...
    @Override
    Set<BasicBlockVertex> getVertices() {
      Set<BasicBlockVertex> set = new HashSet<>();
      for (DecompilerBlock child : allChildren) {
  ...
  private abstract class DecompilerBlock {
    ...
    abstract DecompilerBlock getBlock(BasicBlockVertex vertex);

    abstract Set<BasicBlockVertex> getVertices();
  ...
  private class DecompilerCopy extends DecompilerBlock {
    ...
    BasicBlockVertex getVertex() {
      return (BasicBlockVertex) copy.getRef();
    }
