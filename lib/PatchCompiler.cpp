...
namespace irene3
{
    ...
    std::unique_ptr< llvm::TargetMachine > BuildMachine(
        ...
        auto target_machine = tgt->createTargetMachine(
            triple.getTriple(), cpu, features, options, !no_pic ? llvm::Reloc::PIC_ : llvm::Reloc::Static, std::nullopt, lvl);
    ...
    PatchMetada PatchCompiler::Compile(
        ...
        auto tgt = BuildMachine(triple, this->feature_string, this->cpu, this->no_pic,this->codegen_opt_level);
